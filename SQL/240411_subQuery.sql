-- 서브쿼리(SubQuery) join보다 이해하기 쉽다고 함
/* sql문 내부에서 사용하는 select문을 의미함
   장점 : 복잡한 데이터추출 및 조작작업에 유용하며 유연성과 기능확장에 도움이 됨
   단점 : 지나치게 복잡한 서브쿼리를 사용하면 성능을 저하시킬 수 있음 */

-- 최고의 미일리지를 보유한 고객은 누구인가?
select 고객번호,
	고객회사명,
	담당자명,
	마일리지
from 고객
where 마일리지 = (select max(마일리지) from 고객); -- 이거 익숙해지면 편해보이는 구조상 이거위주로 짤 가능성 높음(주의!)

-- 주문번호 'H0250'을 주문한 고객의 고객회사명과 담당자명
-- 1. 단일 행 SubQuery 사용한 경우
select 고객회사명,	담당자명
from 고객
where 고객번호 = (select 고객번호 from 주문
				where 주문번호 = 'H0250');
			-- 반드시 하나만 나온다는 확신이 있어야 사용가능
			
-- SubQuery 대신 inner join으로 사용한 경우
select 주문번호, 고객회사명, 담당자명
from 고객
inner join 주문
on 고객.고객번호 = 주문.고객번호 
where 주문번호 = 'H0250';			

-- 2. 복수(멀티)행 SubQuery 사용한 경우
-- '부산광역시' 고객이 주문한 주문건수
