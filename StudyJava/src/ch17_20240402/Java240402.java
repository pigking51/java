...
// List : 배열과 가장유사 (주의: 배열은 아님!!!) - 참조형(메모리의 주소 베이스로 정렬되어짐/ 인덱스 중심)
// → 배열은 객체로 인정받지만 클래스로 인정 x(서로 다른 독립된 위치를 각각 기억할 필요 x)
...
// 컬렉션 관련 문제에서 노드 ~ 하는 말 나오면 무조건 이거라고 보면 됨
// (노드 : 하나의 독립된 개체(온전히 하나의 기능을 함)
// → 어떠한 로직의 순서에따라 노드가 이어져서 일련된 작동을 하는것을 노드방식이라고 함
// 배우기 쉽고 쓰기 쉽지만, 크기가 커지면 매우 복잡해짐(노드방식 말하는거)
// → Scratch가 대표적인 노드방식 언어라고 함)

// Set : 비유하자면 보자기처럼 생김 - 집합형()
...
// 정렬방법은 binary Tree라고 함(2진검색트리의 통칭이라고 함) → Red-Black-Tree 
// O(nlogn) ← 이게 가장 좋은 정렬방법이라고 함
// 검색은 logn이 좋음



// Map<K, V> : 한 가지만 제외하고 Set과 매우 유사함
